...
//    获取_scrollView的contentSize
    contentHeight=_scrollView.contentSize.height; // 非刷新状态, 可以在这里判断

//    判断是否在拖动_scrollView
    if (_scrollView.dragging) {
   
        NSLog(@"dragging == yes 正在拖动");
        int currentPostion = _scrollView.contentOffset.y;
        NSLog(@"currentPostion = %zd", currentPostion);
//        判断是否正在刷新  否则不做任何操作
        if (!isRefresh) {
...
//                当currentPostion 小于某个值时 变换状态
                if (currentPostion<-headerHeight*1.5) {
                    ...
                    int currentPostion = _scrollView.contentOffset.y;
//                    判断滑动方向 以让“松开以刷新”变回“下拉可刷新”状态  // 貌似没什么用?
