...
@interface YFContextMenuTableView ()
...
@property (nonatomic, strong) UITableViewCell<YFContextMenuCell> *selectedCell;
...
@implementation YFContextMenuTableView
...
- (void)showInView:(UIView *)superview withEdgeInsets:(UIEdgeInsets)edgeInsets animated:(BOOL)animated {
    ...
    for (UITableViewCell<YFContextMenuCell> *aCell in [self visibleCells]) {
...
- (void)dismisWithIndexPath:(NSIndexPath *)indexPath {
    ...
    for (UITableViewCell<YFContextMenuCell> *visibleCell in visibleCells) {
...
- (void)updateAlongsideRotation {
    if (self.animatingState == Hiding) {
        if ([self.delegate respondsToSelector:@selector(contextMenuTableView:didDismissWithIndexPath:)]) {
            [self.delegate contextMenuTableView:self didDismissWithIndexPath:[self indexPathForCell:self.selectedCell]];
        }
        [self removeFromSuperview];
...
- (void)show:(BOOL)show visibleCellsAnimated:(BOOL)animated {
    ...
    UITableViewCell<YFContextMenuCell> *visibleCell = [visibleCells objectAtIndex:self.animatingIndex];
    if (visibleCell) {
...
- (void)dismissBottomCells {
    if (self.bottomCells.count) {
        UITableViewCell<YFContextMenuCell> *hidingCell = [self.bottomCells lastObject];
...
- (void)dismissTopCells {
    if (self.topCells.count) {
        UITableViewCell<YFContextMenuCell> *hidingCell = [self.topCells firstObject];
...
- (void)prepareCellForShowAnimation:(UITableViewCell<YFContextMenuCell> *)cell {
...
- (void)show:(BOOL)show
        cell:(UITableViewCell<YFContextMenuCell> *)cell
...
- (id)dequeueReusableCellWithIdentifier:(NSString *)identifier forIndexPath:(NSIndexPath *)indexPath {
    ...
    UITableViewCell<YFContextMenuCell> *cell = [super dequeueReusableCellWithIdentifier:identifier forIndexPath:indexPath];
...
#pragma mark - Utils
- (void)resetAnimatedIconForCell:(UITableViewCell<YFContextMenuCell> *) cell{
    UIView *icon = [cell animatedIcon];
...
- (void)setAnchorPoint:(CGPoint)anchorPoint forView:(UIView *)view {
    ...
    //    NSLog(@"view.layer.anchorPoint = %@", NSStringFromCGPoint(view.layer.anchorPoint));
    NSLog(@"oldPoint = %@", NSStringFromCGPoint(oldPoint));
    
    NSLog(@"newPoint transform前 = %@", NSStringFromCGPoint(newPoint));
    newPoint = CGPointApplyAffineTransform(newPoint, view.transform);
    NSLog(@"newPoint transform后 = %@", NSStringFromCGPoint(newPoint));
